...
- **编辑器和游戏版本号独立**：游戏用 v1.xx（index.html），编辑器用 v2.xx（editor/index.html 的 showChangelog）
- **编辑器改动必须更新编辑器更新日志**（editor/index.html 中 showChangelog 的 LOGS 数组），不要改游戏版本号
- **游戏改动必须更新 src/changelog.js**，不要改编辑器版本号
...
## 当前版本
- 游戏: v1.76 (Phase E 自由回访+区域实体懒加载)
- 编辑器: v2.4 (多选系统+组变换+路径渐变+粗细节点共存+折线拐角修复)

## 编辑器版本历史
- v2.4: 多选系统+组变换+路径渐变+粗细节点共存+折线拐角修复+折线增加控制点+坐标修复
- v2.3: 宽度节点系统+增加控制点+Delete删除+切线手柄自由方向+Schneider曲线拟合
- v2.2: 曲线切线权重控制+Catmull-Rom切线
- v2.1: 路径渐变+粗细渐变+可变宽度渲染
- v2.0: 折线和曲线绘制+控制点编辑+变换手柄
