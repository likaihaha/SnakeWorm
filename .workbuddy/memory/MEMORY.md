...
- v1.02: 亲子关系系统（幼体+三叶虫敌人+亲子提示）
- v1.01: 消耗尾巴保留初始速度修复（速度单位修正，乘以60匹配坐标系）
- v1.00: 消耗尾巴保留初始速度修复（保存实际速度而非单位向量）
- v0.99: 消耗尾巴保留初始速度（未生效）
- v0.98: 减速效果增强+视觉反馈
- v0.97: 眼睛位置优化+fireBullet Bug修复
...
## 速度单位规范（2026-04-28）
- 虫虫移动：`velocity * speed * dt * 60`（每秒 `velocity * speed * 60` 像素）
- 宝珠/子弹移动：`velocity * dt * 60`（每秒 `velocity * 60` 像素）
- 尾巴下沉：`velocity * dt`（每秒 `velocity` 像素）
- **关键**：尾巴下沉系统的速度单位比其他系统小60倍，保存速度时需要乘以60
